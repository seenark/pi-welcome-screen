...
/**
 * Load user config from various sources, merged in priority order:
 * 1. Built-in defaults (lowest priority)
 * 2. JSON config file: ~/.pi/welcome-screen.config.json
 * 3. Overrides passed directly (highest priority)
 */
export function loadConfig(overrides: PartialConfig = {}): WelcomeConfig {
...
export function validateConfig(cfg: PartialConfig): string[] {
  const errors: string[] = [];
  const validStyles = ["wave", "rainbow", "glitch", "matrix", "typewriter", "static"];
  ...
  const colorFields: (keyof PartialConfig)[] = [
    "fgColor", "bgColor", "accentColor", "urlColor", "animationColor",
  ];
