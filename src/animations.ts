...
export const BANNER_ONE_LINER = "═══ Code Sook ═══  https://codesook.dev  ══════════════";

// ─── Animation Frame Builders ─────────────────────────────────────────────────

export type FrameBuilder = (bannerLines: string[], frameIndex: number, totalFrames: number) => string[][];
...
function buildWaveFrames(bannerLines: string[], totalFrames: number): string[][] {
  const frames: string[][] = [];
...
function buildRainbowFrames(bannerLines: string[], totalFrames: number): string[][] {
  const colors = ["red", "peach", "yellow", "green", "teal", "sapphire", "blue", "lavender", "pink", "mauve"];
  const frames: string[][] = [];
  for (let f = 0; f < totalFrames; f++) {
    frames.push(
...
function buildGlitchFrames(bannerLines: string[], totalFrames: number): string[][] {
  const frames: string[][] = [];
  for (let f = 0; f < totalFrames; f++) {
    frames.push(
      bannerLines.map((line, lineIdx) => {
...
function buildMatrixFrames(bannerLines: string[], totalFrames: number): string[][] {
  const frames: string[][] = [];
  for (let f = 0; f < totalFrames; f++) {
    frames.push(
      bannerLines.map((line, lineIdx) => {
...
function buildTypewriterFrames(bannerLines: string[], totalFrames: number): string[][] {
  const frames: string[][] = [];
